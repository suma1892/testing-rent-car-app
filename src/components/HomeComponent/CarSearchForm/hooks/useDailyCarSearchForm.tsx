...
const useDailyCarSearchForm = ({
  withDriver,
  reset,
}: UseDailyCarSearchFormProps) => {
  ...
  useEffect(() => {
    ...
    // for DailyListCar page, to restore the input value from Home page
    if (isFocused && !reset) {
      ...
      setForm({
        location: {
          id: formDaily.selected_location?.id,
          name: formDaily.selected_location?.name,
          time_zone: formDaily.selected_location?.time_zone,
          time_zone_identifier:
            formDaily.selected_location?.time_zone_identifier,
        ...
        jam_pengembalian: formDaily.end_booking_time,
      });
    }
  }, [isFocused, reset]);
  ...
  const searchWithDriver = async () => {
    ...
    await analytics().logEvent('search_with_driver', {
  ...
  const searchWithoutDriver = async () => {
    dispatch(
      saveFormDaily({
        ...formDaily,
        limit: 10,
    ...
    navigation.navigate('DailyListCar');
    await analytics().logEvent('search_without_driver', {
      id: new Date().toString(),
      item: 'Daily',
      description: ['without driver'],
      location: form?.location?.name,
      location_id: form?.location?.id,
    });
  };
