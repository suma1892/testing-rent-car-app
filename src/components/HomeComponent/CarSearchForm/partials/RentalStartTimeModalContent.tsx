...
const RentalStartTimeModalContent: FC<RentalStartTimeModalContentProps> = ({
  ...
  function generateTimeLists(selectedHour: string, tgl_sewa: string) {
    ...
    const inputDate = moment(form?.tanggal_sewa, 'YYYY/MM/DD').format(
      'YYYY-MM-DD',
    );
  ...
  useEffect(() => {
    ...
    const data = generateTimeLists(time, form?.tanggal_sewa);
    setListHours(data?.listHours);
    setListMinutes(data?.listMinutes);

    if (isShowBsheet && form?.jam_sewa) {
      const selectedHours = form.jam_sewa.slice(0, form.jam_sewa.length / 2);
      const selectedMinutes = form.jam_sewa.slice(-form.jam_sewa.length / 2);
    ...
    <View style={styles.container}>
      ...
      <Text style={[h4, {textAlign: 'center'}]}>
        ...
        {getIndonesianTimeZoneName({
          timezone: form?.location?.time_zone as any,
