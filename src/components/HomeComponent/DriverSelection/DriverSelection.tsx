...
import {setRentalLocationParams} from 'redux/features/rentalLocation/rentalLocationSlice';
...
const DriverSelection: React.FC<DriverSelectionProps> = React.memo(
  ({onSelect, selected, facilities}) => {
    ...
    const finalSection = useMemo(() => {
      ...
      return buttonList;
    }, [facilities, buttonList]);

    const handlePress = useCallback(
      button => {
        onSelect(button.withDriver);
        dispatch(setRentalLocationParams({facility_id: button?.facility_id}));
      },
      [onSelect, dispatch],
    );
...
export default memo(DriverSelection);
